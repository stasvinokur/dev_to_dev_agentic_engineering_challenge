...
MCP-сервер, который помогает разработчику пройти путь от «quality gate упал» до готового PR — за 5-10 минут, полностью локально.
...
```bash
docker build -f sonar-gatekeeper-mcp/Dockerfile -t sonar-gatekeeper .
```
...
```bash
docker run -d -p 8000:8000 sonar-gatekeeper serve
```
...
Ожидаемый ответ:
```json
{"status":"ok","service":"sonar-gatekeeper","version":"1.0.0","transport":"streamable-http","tools":7}
```
...
Inspector откроется в браузере. Вы увидите 7 инструментов, сможете вызвать любой из них.
...
|----------|-----|-------------|----------|
| `projectKey` | string | да | Ключ проекта SonarQube |
...
```json
{
  "projectStatus": {
    "status": "ERROR",
    "conditions": [
      { "status": "ERROR", "metricKey": "new_reliability_rating", "comparator": "GT", "errorThreshold": "1", "actualValue": "3" }
    ]
  }
}
```
...
| `resolved` | boolean | нет | Фильтр resolved/unresolved (по умолчанию: false) |
...
| `pageSize` | number | нет | Результатов на странице (по умолчанию: 100) |
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|-------------|----------|
| `ruleKey` | string | да | Ключ правила (например `typescript:S1481`) |
...
Найти файл в репозитории по component key из SonarQube и вернуть исходный код с номерами строк.

| Параметр | Тип | Обязательный | Описание |
|----------|-----|-------------|----------|
| `componentKey` | string | да | Component key (например `demo-project:src/utils.ts`) |
| `context` | number | нет | Строк контекста вокруг issues (по умолчанию: 5) |
...
Сгенерировать unified diff патч для файла. **Не записывает файлы** — только возвращает diff.

| Параметр | Тип | Обязательный | Описание |
|----------|-----|-------------|----------|
| `filePath` | string | да | Относительный путь к файлу |
| `original` | string | да | Оригинальный контент для замены |
| `replacement` | string | да | Новый контент |
...
Запуск полного pipeline из 5 агентов. Это основной инструмент для автоматического исправления quality gate.
...
**Результат:** Markdown-отчёт с группировкой issues, патчами, результатами верификации и PR-описанием. Сохраняется в `pipeline-report.md`.
...
1. **Сбор** (без LLM) — собирает данные из SonarQube: статус quality gate, открытые issues, детали правил
2. **Триаж** (LLM) — группирует issues по корневой причине, приоритизирует, выделяет quick wins
3. **Исправление** (LLM) — генерирует минимальные патчи для высокоприоритетных issues
4. **Верификация** (без LLM) — запускает тесты, линтинг, форматирование для проверки патчей
...
- **Языки**: TypeScript / JavaScript проекты. Патчи генерируются только для TS/JS файлов.
- **SonarQube**: self-hosted Community Edition. SonarCloud не поддерживается.
- **LLM**: `qwen2.5-coder:1.5b` — маленькая модель (~1 ГБ). Качество патчей ограничено размером модели; для более сложных задач рекомендуется модель покрупнее.
- **Тестовый проект**: `demo_project/` — 4 файла TypeScript с намеренными code smells (20 КБ).
- **Docker image**: 333 МБ (лимит 500 МБ).
- **Ресурсы**: работает при CPU 2.0 / RAM 2048 МБ.
- **Секреты**: токен SonarQube генерируется автоматически, ручная настройка не требуется.

---

## Расширенный режим (опционально)

Для полного демо-сценария с SonarQube, Ollama и трассировкой — используйте Docker Compose.
...
`--wait` дожидается пока указанные сервисы станут healthy. Остальные запускаются как зависимости. Поднимает:
...
| `SONAR_TOKEN` | нет | авто | Токен SonarQube (генерируется автоматически) |
...
| `PROJECT_ROOT` | нет | `process.cwd()` | Корень анализируемого репозитория |

Шаблон: [.env.example](sonar-gatekeeper-mcp/.env.example)

### Langfuse трассировка

После запуска pipeline откройте http://localhost:3000 — в Langfuse видны трассы pipeline с отдельными spans для каждого агента (Сбор, Триаж, Исправление, Верификация, Отчёт) — с входными/выходными данными, ошибками и таймингами.

---

## Архитектура

Двухслойный дизайн: **инструментальный слой** (чистый доступ к данным) и **агентный слой** (pipeline с LLM).
...
- **LLM**: Vercel AI SDK 6 + `@ai-sdk/openai-compatible` (провайдер Ollama)
- **Валидация**: Zod
