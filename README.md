...
```
Collector --> Triage --> Fix --> Verifier --pass--> Reporter --> Результат
                                    |
                                  fail --> Fix (retry, макс. 2 попытки)
```

1. **Collector** (без LLM) — собирает данные из SonarQube: статус quality gate, открытые issues, детали правил
2. **Triage** (LLM) — группирует issues по корневой причине, приоритизирует, выделяет quick wins
3. **Fix** (LLM) — генерирует минимальные патчи для высокоприоритетных issues
4. **Verifier** (без LLM) — запускает тесты, линтинг, форматирование для проверки патчей
5. **Reporter** (LLM) — генерирует PR-описание с итогами
...
После запуска pipeline откройте http://localhost:3000 — в Langfuse видны трассы всех LLM-вызовов (Triage, Fix, Reporter) с входными/выходными данными и таймингами.
